...
function checkAnswer(event) {
    ...
    if (answer === correctAnswer) {
        ...
        if (currentQuestion == allQuestions.length -1) {
            // call function to move to high score page      
        }else {
            currentQ++;
            displayQuestion()   
        }
        ...
        if (currentQuestion == allQuestions.length -1) {
            // call function to move to high scre page
    ...
    if (currentQuestion < questionNumbers.length) {
        displayQuestion(startTime);
    } else {
        // then when done throw info to initials box to show quiz results
        remainingTime();
    }
};
